...
function ChannelsManager(server) {
    ...
    const channelsConfig = serverConfigUtils.getConfig('endpointsConfig', 'virtualMQ', 'channelsFolderName');
    const channelKeyFileName = "channel_key";

    const rootFolder = path.join(serverConfigUtils.getConfig('storage'), channelsConfig);
    ...
    if (integration.testIfAvailable()) {
        forwarder = integration.getForwarderInstance(serverConfigUtils.getConfig('zeromqForwardAddress'));
    }
